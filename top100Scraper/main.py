...
imageC = customtkinter.CTkImage(Image.open(myPath +"/image_processing20200424-23346-xarvu8.png"), size=(25,25))
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        fromFile = customtkinter.CTkButton(master=frame, text="Import From File...", command=self.chooseFile, width=100, height=50, image=imageC)
        fromFile.pack(pady=15,padx=15)
    ...
    def generatePlaylistFromFile(self,fileName):
        playlist = scrapeAndMake()
        playlist.addSongsFromFile(fileName)
    ...
    def loadingFile(self, filePath):
        ...
        text = customtkinter.CTkLabel(master=frame, text="Creating playlist.", font=("Roboto", 30))
        text.pack(pady=15, padx=15)
        thread = threading.Thread(target=self.generatePlaylistFromFile(filePath))
        thread.daemon = True
        thread.start()
        x = 0
        loadScreen.update_idletasks()
